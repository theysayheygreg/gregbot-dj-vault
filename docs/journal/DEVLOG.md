...
## 2026-04-20 — VaultBuddy Becomes A Sandbox Operator

The important UI move today was making `sandbox-v1` feel like the actual target library instead of a preparation ritual off to the side. VaultBuddy now defaults the desktop runtime to the generated sandbox database, exposes a Prepare Target Library action, and shows the `canonical-embedded :: Warmup Tools` export-readiness proof directly in the Exports view. That gives the product a visible loop: rebuild the sandbox, inspect whether the USB-shaped export is ready, and keep the native gaps named without hiding them.

The other useful move was turning trust-aware playlist retrieval into a first-class surface. The new Candidates view takes a DJ-shaped prompt, a mode like `gig-safe` or `discovery`, and emits a persisted Markdown/JSON candidate report. This is still deterministic scoring, not vector search magic, but it creates the interaction we wanted to test: trust, recency, and musical fit are visible as playlist-shaping inputs rather than buried as backend diagnostics.

The whole-product review also clarified two next risks. Device export currently writes the expected files but does not prune stale files from a previous export root, which is not safe enough for real USB media. Playback history import also increments play counts on every import with no idempotency key, which will distort recency if a DJ imports the same Rekordbox or Traktor history twice. Those are product bugs, not polish notes, because they affect what a DJ trusts before a gig.

Those two risks were fixed in the next pass. The device exporter now reads the previous DJ Vault manifest, removes the staged media it owns, and wipes only DJ Vault's own metadata folder before writing the next export. It intentionally does not erase an entire USB root blindly. Playback history import now has a source-identity uniqueness rule and duplicate imports are ignored instead of counted again. The sandbox export regression now proves both behaviors: stale media disappears, and a second import of the same history payload records one duplicate and zero new events.
